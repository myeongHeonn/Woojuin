...
/**
 * 개인정보처리방침 본문 — PrivacyPolicyPage(단독 페이지)와
 * PrivacyPolicyConsentModal(회원가입 동의 모달)이 같은 본문을 공유한다.
...
const PrivacyPolicyContent = () => (
  <>
    <p className="text-[13px] text-text-3">시행일: 2026년 8월 1일</p>

    <p className="mt-6 leading-relaxed text-text-2">
      우주인 팀(이하 &ldquo;팀&rdquo;)은 올인원 AI 스크랩북 서비스 &lsquo;우주인&rsquo;(이하
      &ldquo;서비스&rdquo;)을 운영하며, 「개인정보 보호법」 등 관계 법령에 따라 이용자의 개인정보를
      보호하고 이와 관련한 고충을 신속하게 처리하기 위하여 다음과 같이 개인정보처리방침을
      수립·공개합니다. 본 서비스는 삼성 청년 SW·AI 아카데미(SSAFY) 교육 과정에서 개발·운영됩니다.
    </p>
    ...
    <Section title="제3조 (개인정보의 처리 및 보유 기간)">
      <p>
        팀은 법령에 따른 보유 기간 또는 수집 시 동의받은 보유 기간 내에서 개인정보를 처리·보유하며,
        기간이 지나면 지체 없이 파기합니다.
      </p>
      <PolicyTable
        head={['항목', '보유 기간']}
    ...
    <Section title="제4조 (개인정보의 제3자 제공)">
      <p>
        ① 팀은 이용자의 개인정보를 제1조의 목적 범위에서만 처리하며, 이용자의 별도 동의 또는 법령의
        특별한 규정이 있는 경우를 제외하고는 제3자에게 제공하지 않습니다.
      </p>
      <p>
        ② 다만 이용자가 워크스페이스 초대 링크를 다른 사람에게 공유하여 그 사람이 워크스페이스에
        참여하면, 참여한 멤버는 해당 워크스페이스에 저장된 모든 항목(본문·위치정보 포함)을 열람할 수
        있습니다. 이는 이용자의 선택에 따른 공개이므로 초대 링크 공유 시 유의하시기 바랍니다.
      </p>
    </Section>
    ...
    <Section title="제5조 (개인정보 처리의 위탁 및 국외 이전)">
      <p>
        ...
        수탁자는 국외 사업자입니다. 국외 이전은 해당 기능을 이용하는 시점에 정보통신망을 통해 수시로
        이루어집니다.
      </p>
      <PolicyTable
      ...
      />
      <p>
        ② AI 처리·소셜 로그인·푸시 알림은 서비스의 본질적 기능이므로, 국외 이전을 거부하실 경우 해당
        기능 또는 서비스 전체의 이용이 제한될 수 있습니다. 이전을 원하지 않으시면 제10조의 문의처로
        연락해 주시기 바랍니다.
      </p>
    </Section>
    ...
    <Section title="제8조 (개인정보의 안전성 확보 조치)">
      <ol className="flex list-decimal flex-col gap-1 pl-5">
        ...
        <li>
          인증 토큰의 단기 만료(접근 토큰 1시간, 재발급 토큰 14일) 및 새 로그인 시 기존 재발급 토큰
          즉시 무효화
        </li>
    ...
    <Section title="제9조 (쿠키 등 자동 수집 장치)">
      ...
      <p>
        ② 로그인 상태 유지를 위해 인증 토큰을 이용자의 브라우저 저장 공간(localStorage)에 보관하며,
        로그아웃하면 삭제됩니다. 공용 PC에서는 이용 후 반드시 로그아웃해 주시기 바랍니다.
      </p>
      <p>
    ...
    <Section title="제12조 (개인정보처리방침의 변경)">
      <p>
        이 방침의 내용이 추가·삭제·수정되는 경우 시행 7일 전(이용자 권리의 중요한 변경은 30일 전)에
        서비스 내 공지로 알립니다.
      </p>
