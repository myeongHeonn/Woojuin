...
실제 임베딩 문자열은 카테고리를 ID 순으로 정렬한 뒤 다음 구조로 고정합니다. 라벨
없는 평문인 이유는 검색어 임베딩(생 문장)과 형태를 맞추기 위해서입니다 — 라벨을
붙이면 모든 아이템이 같은 보일러플레이트를 공유해 관련/무관 거리 차가 좁아집니다.

```text
참깨와 파를 곁들인 소바 국물면
참깨와 파 등의 고명이 올라간 일본식 소바 국물면이다.
음식·맛집
```
