...
public class KoreanAddressExtractor {
    ...
    private static final Pattern KOREAN_ADDRESS = Pattern.compile(
            ...
            //    토큰을 1~2회 반복하는 건 통합 광역단체를 위한 것이다. 카카오는 이미
            //    "전남광주통합특별시"로 내려주는데, 토큰 하나만 허용하면 '전남' 다음에 공백이
            //    없어서(바로 '광주') 매칭이 통째로 실패한다. 알려진 토큰 두 개의 결합만
            //    허용하므로 임의의 한글이 새로 붙지는 않는다 — '대구경북', '부산경남' 같은
            //    향후 통합도 자동으로 커버된다.
            "(?:서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충청북도|충남|충청남도|"
            + "전북|전라북도|전남|전라남도|경북|경상북도|경남|경상남도|제주){1,2}"
            + "(?:통합특별시|특별자치시|특별자치도|특별시|광역시|도)?"
