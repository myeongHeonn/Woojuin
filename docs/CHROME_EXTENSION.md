...
익스텐션에는 **자체 로그인 폼이 없습니다.** 웹앱의 구글 로그인을 그대로 쓰고 그 세션을
물려받습니다.

- 웹앱은 이메일·비밀번호 로그인을 쓰지 않습니다(`LoginForm.tsx`에서 주석 처리). 구글뿐이고,
  구글은 첫 로그인이 곧 가입입니다(`OAuthAccountService.findOrCreateUser`). 따라서 실사용자는
  `password_hash`가 없어 `/auth/login`으로는 통과할 수 없습니다.
- 익스텐션 안에서 OAuth를 직접 하려면(`chrome.identity`) 백엔드가 익스텐션용 `redirect_uri`를
  받아 주고 구글 콘솔에도 등록해야 합니다. 세션을 물려받으면 그 변경이 전부 불필요합니다.

동작 순서:

1. 팝업의 `Google 계정으로 로그인`을 누르면 먼저 **열려 있는 우주인 탭에서 세션을 찾습니다.**
...
- `scripting` — 우주인 웹앱 탭에서 사용자의 로그인 세션을 읽어 오기 위해 사용합니다. 자체 로그인
  폼 없이 웹앱의 구글 로그인 결과를 물려받으므로 인증 토큰만 조회하며, 다른 사이트나 페이지
  내용에는 접근하지 않습니다.
...
익스텐션은 웹앱의 로그인 세션을 물려받습니다. 최초 실행 시 `Google 계정으로 로그인`을 누르면
됩니다(계정이 없어도 구글 로그인이 곧 가입입니다). 저장할 곳은 팝업의 선택기에서 고르며,
`Personal Space`와 `Workspaces`가 웹앱 사이드바와 같은 방식으로 구분됩니다. 이미지 우클릭
저장만은 팝업을 거치지 않고 메뉴에서 바로 고를 수 있습니다(아래 `이미지 저장` 참고).
...
1. 이미지에서 우클릭
2. `선택한 이미지를 우주인에 저장` → **저장할 스페이스 선택**
3. 최초 출처 권한 요청 허용
4. Chrome의 `우주인 저장 완료` 알림 확인

이미지는 보던 페이지에서 바로 담는 흐름이라, 저장할 곳을 **우클릭 메뉴에서 바로 고릅니다.**
하위 메뉴에 스페이스가 펼쳐지고 고른 곳으로 들어갑니다. 팝업 선택기와 같은 순서·이름으로
표시되며(`Personal Space`가 먼저, 구분선 뒤에 팀 워크스페이스), 접수 문구에 `○○에 보냈어요`로
어디에 담겼는지 나옵니다.

여기서 고른 곳은 **그 저장 한 번에만** 적용됩니다. 팝업의 기본 저장 위치는 그대로입니다.

스페이스가 하나뿐이거나 아직 목록을 받지 못했으면(로그인 전) 하위 메뉴 없이 단일 항목으로
표시되고, 팝업에서 고른 곳으로 저장됩니다. 텍스트 저장은 팝업에서 고른 곳을 그대로 씁니다.

지원 이미지는 JPG, PNG, GIF, WEBP이며 최대 크기는 10MB입니다. 저장 결과는 저장한 워크스페이스의
라이브러리에서 확인합니다. 저장 요청은 즉시 접수되고 AI 제목·요약·카테고리 처리는
백그라운드에서 진행됩니다.
...
1. 구글 로그인 — 작은 창이 뜨고, 끝나면 자동으로 닫히며 팝업이 로그인 상태로 바뀜
2. 저장할 곳 선택기에 `Personal Space`와 `Workspaces`가 구분되어 표시됨
3. 현재 페이지 저장
4. 선택한 텍스트 우클릭 저장
5. 선택한 이미지 우클릭 저장 — 하위 메뉴에 스페이스가 뜨고, 고른 곳이 접수 문구에 표시됨
6. 팀 워크스페이스를 고른 뒤, 팝업의 기본 저장 위치가 바뀌지 않았는지 확인
7. 웹 라이브러리에서 저장 및 AI 처리 결과 확인
